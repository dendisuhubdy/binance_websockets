...
void fetch_snapshot(const std::string& symbol, int limit) {
    try {
        ...
        http::request<http::string_body> req{http::verb::get, "/api/v3/depth?symbol=" + to_upper(symbol) + "&limit=" + std::to_string(limit), 11};
...
void print_order_book(int depth = 10) {
    std::cout << "\n===== ORDER BOOK (Top " << depth << " levels) =====" << std::endl;
    ...
    for (auto it = asks.begin(); it != asks.end() && count < depth; ++it, ++count) {
        std::cout << std::setw(16) << it->first << " | " << std::setw(14) << it->second << std::endl;
    }
...
int main() {
    std::string symbol = "btcusdt";  // Will be converted to uppercase
    int limit = 1000; // Depth for snapshot

    // Step 1: Fetch initial snapshot
    std::cout << "Fetching initial snapshot for " << to_upper(symbol) << "..." << std::endl;
    ...
    try {
        ...
        // WebSocket endpoint uses lowercase symbol
        std::string ws_symbol = symbol;
        std::transform(ws_symbol.begin(), ws_symbol.end(), ws_symbol.begin(), 
                      [](unsigned char c){ return std::tolower(c); });
        
        std::cout << "Connecting to WebSocket stream..." << std::endl;
        ws.handshake("stream.binance.com:9443", "/ws/" + ws_symbol + "@depth@100ms");
        std::cout << "Connected! Receiving depth updates..." << std::endl;
        ...
        while (true) {
            ...
            if (data["e"] == "depthUpdate") {
                apply_update(data);
                // For demo, print every 10 updates
                static int update_count = 0;
                if (++update_count % 10 == 0) {
                    print_order_book(20); // Print top 10 levels as requested
                }
            }
        }
